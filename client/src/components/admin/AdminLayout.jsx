...
const ROLE_META = {
  ...
  // Fee Ledger module roles (server/models/User.js ROLES) — currently
  // unreachable in practice (the module is hard-gated to admin-only, see
  // server/src/modules/fees/routes/index.js), kept here so this doesn't
  // silently regress to the counsellor fallback styling if that gate is
  // ever lifted. registrar is college finance/admissions staff, partner is
  // a recruitment agency login.
  registrar: {
    ...
    iconActiveClass: 'text-emerald-700',
  },
  partner: {
    ...
    <div className="min-h-screen bg-secondary/30 sm:flex">
      ...
      >
        ...
        <div className="flex items-center gap-2.5 border-b px-5 py-5">
          <span className={cn('flex h-9 w-9 shrink-0 items-center justify-center rounded-md text-white text-xs font-bold', roleMeta.tileClass)}>
            CIC
          </span>
      ...
      <div className="flex min-h-screen flex-1 flex-col sm:min-w-0">
        ...
        <div className="sticky top-0 z-20 flex h-14 items-center gap-3 border-b bg-white px-4 sm:hidden">
          ...
          <span className="font-bold text-sm text-gray-900">Capital Immigration Canada</span>
        </div>
